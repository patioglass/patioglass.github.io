                ...
                $.getJSON("./illustrator.json",function(data){
                    $.each(data,function(i,value){
                        if(month==value.month && year==value.year){
                            /*トップ絵が複数ある場合*/
                            num=Math.floor(Math.random()*value.number)+1;
                            img_url="./img/top_picture/"+value.year+"/"+value.year+"_"+value.month+"_"+num+".jpg";
                            empty_flag=false;
                            array.push({
                                name:value.name[num-1].name,
                                id:value.name[num-1].id,
                                month:value.month,
                                year:value.year,
                                append_flag:true,
                                img:img_url,
